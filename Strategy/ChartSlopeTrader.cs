...
namespace NinjaTrader.Strategy
{
    ...
    public class ChartSlopeTrader : Strategy
    {
        ...
        private ComboBox _comboBoxOrderType;
 		...
 		private void VS2010_InitializeComponent_Form()
        {
			...
			_comboBoxOrderType = new ComboBox();
...
// 
	        ...
	        // comboBox_OrderType
	        // 
	        _comboBoxOrderType.DropDownStyle = ComboBoxStyle.DropDownList;
	        _comboBoxOrderType.FormattingEnabled = true;
	        _comboBoxOrderType.Items.AddRange(new object[] {
            "LIMIT",
            "MARKET"});
	        _comboBoxOrderType.SelectedIndex=0;
	        _comboBoxOrderType.Location = new Point(80, 17);
	        _comboBoxOrderType.Margin = new Padding(2);
            _comboBoxOrderType.Name = "comboBox_OrderType";
            _comboBoxOrderType.Size = new Size(66, 21);
            _comboBoxOrderType.TabIndex = 4;
            // 
            ...
            _groupBoxMode.Controls.Add(_comboBoxOrderType);
       ...
       private Button _buttonActivate;
	    ...
	    private void UpdateOrders()
	    {
		    ...
		    else
		    {
			    ...
			    if (!_currentRayContainer.ClosingHulf)
			    {
				    if (_currentRayContainer.PositionType == MarketPosition.Short)
				    {
					    if (Close[0] < RayContainer.RayPrice(_currentRayContainer.HalfCloseRay))
					    {
						    ...
						    _currentRayContainer.ClosingHulf = false;
					    }
				    }
				    else
				    {
				...
				SetTrailStop("TrailingStopCST",CalculationMode.Price,RayContainer.RayPrice(_currentRayContainer.ProfitTargetRay),false);
	    ...
	    private void OrderTriggerEnterLongStop()
	    {
		    ...
		    _currentOrder = EnterLongStopLimit(quantity, stopPrice+LimitShift,stopPrice);
	    }
	    ...
	    private void OrderTriggerEnterShortStop()
	    {
		    int quantity = (int) _numericUpDownQuantity.Value;
		    double stopPrice = RayContainer.RayPrice(_currentRayContainer.EntryRay);
		    _currentOrder = EnterShortStopLimit(quantity, stopPrice- LimitShift,stopPrice);
	    }
	    ...
	    private void UpdateRr()
	    {
		    ...
		    if (_currentRayContainer!=null)
		    {
			    ...
			    if (reward != 0)
				    _rrLabel.Text = Math.Round((risk/reward),2).ToString();
				//For 50% RR 
			    if (_currentRayContainer.ClosingHulf)
			    {
				    if (MarketPosition.Flat == Position.MarketPosition)
					...
					if (reward != 0)
						_rr50Label.Text = Math.Round((risk / reward), 2).ToString();
	...
	{
		private static readonly Color _enterColor = Color.DarkRed;
		private static readonly Color _tpColor = Color.Lime;
		private static readonly Color _stopColor = Color.Red;
		private static readonly Color _hcColor = Color.Purple;
		...
		private readonly Color _textColror = Color.Black;
		private IDot _eDot;
		...
		public void Update()
		{
			...
			_eText = _strategy.DrawText("enterText", TextForma(s), 0, RayPrice(EntryRay), _textColror);

			//For stop Ray
			_sDot = _strategy.DrawDot("stopDot", true, 0, RayPrice(StopRay), _dotColor);
			double text = RayPrice(StopRay);
			_sText = _strategy.DrawText("stopText", TextForma(text), 0, RayPrice(StopRay), _textColror);

			//For TP Ray
			_tpDot = _strategy.DrawDot("TPDot", true, 0, RayPrice(ProfitTargetRay), _dotColor);
			double priceText = RayPrice(ProfitTargetRay);
			_tpText = _strategy.DrawText("TPText", TextForma(priceText), 0, RayPrice(ProfitTargetRay), _textColror);
			if (ClosingHulf)
			{
				_hcDot = _strategy.DrawDot("HCDot", true, 0, RayPrice(HalfCloseRay), _dotColor);
				double priceT = RayPrice(HalfCloseRay);
				_hcText = _strategy.DrawText("HCText", TextForma(priceT), 0, RayPrice(HalfCloseRay), _textColror);
			}
		}

		private string TextForma(double price)
		{
			string priceText = Math.Round(price, 4).ToString();
